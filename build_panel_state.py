...
def initalize_state_onset_panel(panel_start_year, panel_end_year, telecon_path, pop_path, clim_index, response_var, plot_telecon=False):
    ...
    if (plot_telecon==True):
        ...
        bounds = [np.min(last_obs['pop_avg_psi']), 0.36614443, np.max(last_obs['pop_avg_psi'])]
    ...
    cols = [col for col in panel_gdf.columns if col not in ['geometry', 'pop2000']] # COMMENT OUT IF YOU WANT TO KEEP THE GEOMETRY COLUMN
    panel_gdf = panel_gdf[cols]                                                     # COMMENT OUT IF YOU WANT TO KEEP THE GEOMETRY COLUMN
...
panel = initalize_state_onset_panel(panel_start_year=1950,
                                    panel_end_year=2024,
                                    telecon_path = '/Users/tylerbagwell/Documents/Rice_University/CCCV/data/cccv_data/processed_teleconnections/psi_NINO3_type1.nc',
                                    ...
                                    response_var = 'binary',
                                    plot_telecon=True)
panel.to_csv('/Users/tylerbagwell/Documents/Rice_University/CCCV/data/panel_datasets/onset_datasets_state/Onset_Binary_GlobalState_NINO3type1_newonsetdata.csv', index=False)
