...
#write.csv(dat_agg, file = "/Users/tylerbagwell/Desktop/panel_datasets/results_for_onsets/CE_cindex_lag0y_Onset_Count_Global_NINO3type2_square4.csv", row.names = FALSE)

mod0 <- lm(conflict_count ~ year + bool1989, data=dat_agg)
mod1 <- lm(conflict_count ~ cindex_lag0y + cindex_lag1y + year + bool1989, data=dat_agg)
summary(mod1)
...
plot(ce, ask = FALSE)
ce_data <- ce$cindex_lag0y
write.csv(ce_data, file = "/Users/tylerbagwell/Desktop/panel_datasets/results/CE_cindex_lag0y_Onset_Count_Global_NINO3type2_square4_strong_ci90_poisson.csv", row.names = FALSE)
...
thresh_form <- bf(
  conflict_binary ~ inv_logit(eta),                    # Bernoulli link
  eta ~ alpha                                          # intercept
  + beta * I(cindex_lag0y^2) * step(psi - tau)               # effect ON only above
  + gamma * year                                 # fixed year trend
  + (1 + year | loc_id),                         # same RE structure
  alpha ~ 1, beta ~ 1, gamma ~ 1, tau ~ 1,             # models for nl-pars
  nl   = TRUE
)
...
thresh_form <- bf(
  ...
  conflict_binary ~ alpha +                              # baseline log-odds
    beta * I(cindex_lag0y^2) * step(psi - tau)       # effect starts above τ
    gamma * year +
    (1 + year | loc_id),                 # same RE block as before
  
  # --- sub-formulas telling brms these are non-linear parameters -----------
  alpha ~ 1,
  beta  ~ 1,
  gamma ~ 1,
  tau   ~ 1,
  
  nl = TRUE                                              # <-- the crucial flag
)

## 2.  Priors ---------------------------------------------------------------
priors <- c(
  prior(normal(0, 2.5), nlpar = "alpha"),                # intercept on logit scale
  prior(normal(0, 1),   nlpar = "beta"),                 # slope above τ
  prior(normal(0, 1),   nlpar = "gamma"),                # year trend
  prior(uniform(0, max(dat$psi)), nlpar = "tau")                   # τ within ±3 SD of predictor
)

## 3.  Fit ------------------------------------------------------------------
fit <- brm(
  conflict_binary ~ I(cindex_lag0y^2) * step(psi - tau),
  data   = dat,
  family = bernoulli(link = "logit"),                    # default logit link
