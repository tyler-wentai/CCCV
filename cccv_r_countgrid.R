...
panel_data_path <- '/Users/tylerbagwell/Desktop/panel_datasets/onset_datasets_grid/Onset_Count_Global_NINO3finalspi6_square4.csv'
...
dat_help <- subset(dat, psi > 1.45043932)
...
mod1 <- lm(conflict_count ~ I(cindex_lag0y^1) + I(cindex_lag1y^1) + year + bool1989, data=dat_agg)
summary(mod1)
plot(dat_agg$cindex_lag1y, dat_agg$conflict_count)
abline(mod1)
...
plot(ce, ask = FALSE)
ce_data <- ce$cindex_lag0y
write.csv(ce_data, file = "/Users/tylerbagwell/Desktop/panel_datasets/results/CE_cindex_lag0y_Onset_Count_Global_DMI_square4_high95_poisson.csv", row.names = FALSE)
...
fit_lin <- brm(
  ...
  iter = 5000, chains=2, warmup=1000, cores=1,
  prior = c(
    prior(normal(0, 2.5), class = "b"),                # regression coefficients
    prior(normal(0, 5), class = "Intercept"),          # intercept term
...
plot(fit_lin)
print(summary(fit_lin, prob = 0.95), digits = 4)


ce <- conditional_effects(fit_lin, effects = "cindex_lag0y", prob = 0.95, resolution = 1000)
plot(ce, ask = FALSE)
ce_data <- ce$cindex_lag0y
write.csv(ce_data, file = "/Users/tylerbagwell/Desktop/panel_datasets/results/CE_cindex_lag0y_Onset_Count_Global_DMI_square4_high95_linear.csv", row.names = FALSE)
