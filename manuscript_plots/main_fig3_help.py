...
path_psi_eff_pos = "/Users/tylerbagwell/Desktop/margeffect_psi.csv"
path_psi_eff_neg = "/Users/tylerbagwell/Desktop/margeffect_psi_negiod.csv"

psi = pd.read_csv(path_psi)
psi_eff_pos = pd.read_csv(path_psi_eff_pos)
psi_eff_neg = pd.read_csv(path_psi_eff_neg)
...
# -- Panel 3 (top-right) --
ax2 = ax1.twinx()
ax1.set_title('IOD & Conflict, Dose-response, State', fontsize=12)

# hist + lines
sns.histplot(x=psi['pop_avg_psi'], color='gainsboro',
             stat='proportion', bins="scott", alpha=1.0, zorder=3,
             ax=ax1)
sns.lineplot(x='pop_avg_psi', y='estimate', data=psi_eff_pos,
             color='crimson', ax=ax2, zorder=4)
ax2.fill_between(psi_eff_pos['pop_avg_psi'],
                 psi_eff_pos['conf.low'], psi_eff_pos['conf.high'],
                 color='crimson',
                 alpha=0.20, edgecolor=None)
# sns.lineplot(x='pop_avg_psi', y='estimate', data=psi_eff_neg,
#              color='blue', ax=ax2)
# ax2.fill_between(psi_eff_neg['pop_avg_psi'],
#                  psi_eff_neg['conf.low'], psi_eff_neg['conf.high'],
#                  color='blue',
#                  alpha=0.20, edgecolor=None)
# ax2.yaxis.set_major_formatter(mticker.FormatStrFormatter('%.2f'))
import matplotlib.ticker as mtick
ax2.yaxis.set_major_formatter(mtick.FuncFormatter(lambda y, _: f"{y*100:.0f}"))
ax2.axhline(0.0, color='k', linestyle="--", zorder=0)


ax2.plot([0.43, 0.43], [-0.0258, 0.0165], color='gray', linestyle='-', linewidth=0.9, zorder=2) # Ethiopia
ax2.plot([0.66, 0.66], [-0.0290, 0.0353], color='gray', linestyle='-', linewidth=0.9, zorder=2) # Somalia
ax2.plot([0.73, 0.73], [-0.0275, 0.042], color='gray', linestyle='-', linewidth=0.9, zorder=2) # Indonesia

plt.text(0.435, -0.02, "Ethiopia", rotation=270, color='gray', fontsize=7)
plt.text(0.665, -0.02, "Somalia", rotation=270, color='gray', fontsize=7)
plt.text(0.735, -0.02, "Indonesia", rotation=270, color='gray', fontsize=7)
...
ax1.set_yticklabels([0, 0.1, 0.2, 0.3], fontsize=9)
ax1.set_ylabel("Proportion", ha='right', fontsize=9)
ax1.set_xlabel(r"IOD teleconnection strength, $\Psi$", fontsize=11)
ax1.tick_params(axis='y', direction='in')
ax1.yaxis.set_label_coords(1.1, 0.22)

ax2.yaxis.tick_left()
ax2.yaxis.set_label_position("left")
ax2.tick_params(axis='y', direction='in')
ax2.set_ylabel(r"Change in onset probability""\n(+IOD - neutral), p.p.", fontsize=12)



# annotations
ax1.text(-0.04, 1.02, 'c', transform=ax1.transAxes, ha="center", va="center",
...
ax2.xaxis.set_major_locator(FixedLocator(pct_to_x(pct_ticks)))
ax2.xaxis.set_major_formatter(mtick.FormatStrFormatter('%.2f'))

# secondary axis
secax = ax2.secondary_xaxis(-0.2, functions=(x_to_pct, pct_to_x))
...
secax.set_xlabel(r"Percentile of IOD teleconnection strength, $\Psi$")
