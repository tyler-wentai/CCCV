...
path1_h = '/Users/tylerbagwell/Documents/Rice_University/CCCV/data/panel_datasets/results_for_onsets/cindex_lag0y_Onset_Binary_GlobalState_NINO3type2_strong_ci90_linear.csv'
...
path1_l = '/Users/tylerbagwell/Documents/Rice_University/CCCV/data/panel_datasets/results_for_onsets/cindex_lag0y_Onset_Binary_GlobalState_NINO3type2_weak_ci90_linear.csv'
...
# psi_eff_neg["estimate"] = - psi_eff_neg["estimate"]
# psi_eff_neg["conf.low"] = - psi_eff_neg["conf.low"]
# psi_eff_neg["conf.high"] = - psi_eff_neg["conf.high"]
...
ax2.set_ylabel('ACR (p.p.)', fontsize=11)
...
ax4.set_ylabel('ACR (p.p.)', fontsize=11)
...
sns.lineplot(x='pop_avg_psi', y='estimate', data=psi_eff_pos_strong, linestyle='-',
             color='#B91C1C', ax=ax6, label=r"+$0.97^\circ$C (Max obs.)")
sns.lineplot(x='pop_avg_psi', y='estimate', data=psi_eff_pos,
             color='#EA580C', ax=ax6, zorder=4, label=r"$+0.80^\circ$C (2 s.d.)")
ax6.fill_between(psi_eff_pos['pop_avg_psi'],
...
sns.lineplot(x='pop_avg_psi', y='estimate', data=psi_eff_pos_weak, linestyle='-',
             color='#F59E0B', ax=ax6, label=r"$+0.40^\circ$C (1 s.d.)")
# ax6.fill_between(psi_eff_neg['pop_avg_psi'],
#                  psi_eff_neg['conf.low'], psi_eff_neg['conf.high'],
#                  color='blue',
#                  alpha=0.20, edgecolor=None)
# ax6.yaxis.set_major_formatter(mticker.FormatStrFormatter('%.2f'))
import matplotlib.ticker as mtick
ax6.yaxis.set_major_formatter(mtick.FuncFormatter(lambda y, _: f"{y*100:.0f}"))
ax6.axhline(0.0, color='k', linestyle="--", zorder=0)


ax6.plot([0.43, 0.43], [-0.0258, 0.0227], color='grey', linestyle='-', linewidth=0.8, zorder=2) # Ethiopia
ax6.plot([0.66, 0.66], [-0.0290, 0.0525], color='grey', linestyle='-', linewidth=0.8, zorder=2) # Somalia
ax6.plot([0.73, 0.73], [-0.0278, 0.0650], color='grey', linestyle='-', linewidth=0.8, zorder=2) # Indonesia

ax6.scatter([0.43], [0.0230], color='#B91C1C', s=10, zorder=2) # Ethiopia
ax6.scatter([0.66], [0.0528], color='#B91C1C', s=10, zorder=2) # Somalia
ax6.scatter([0.73], [0.0652], color='#B91C1C', s=10, zorder=2) # Indonesia

ax6.scatter([0.43], [0.0142], color='#EA580C', s=10, zorder=2) # Ethiopia
ax6.scatter([0.66], [0.0290], color='#EA580C', s=10, zorder=2) # Somalia
ax6.scatter([0.73], [0.0348], color='#EA580C', s=10, zorder=2) # Indonesia

ax6.scatter([0.43], [0.0035], color='#F59E0B', s=10, zorder=2) # Ethiopia
ax6.scatter([0.66], [0.0056], color='#F59E0B', s=10, zorder=2) # Somalia
ax6.scatter([0.73], [0.0063], color='#F59E0B', s=10, zorder=2) # Indonesia
...
ax6.set_yticks([0, 0.02, 0.04, 0.06])
ax6.set_yticklabels(["0", "+2", "+4", "+6"], fontsize=11)
ax6.tick_params(axis='y', direction='in')
ax6.set_ylabel(r"Change in onset probability,""\n+IOD - neutral (p.p.)", fontsize=11)
...
ax6.set_xlim(xmin, 0.9)
ax6.xaxis.set_major_locator(FixedLocator(pct_to_x(pct_ticks)))
ax6.xaxis.set_major_formatter(mtick.FormatStrFormatter('%.2f'))

# secondary axis
secax = ax6.secondary_xaxis(-0.2, functions=(x_to_pct, pct_to_x))
...
ax8 = ax7.twinx()
ax7.set_title('IOD & Conflict, State', fontsize=12)

sns.histplot(x=df_c3['cindex_lag0y'], color='gainsboro',
             stat='proportion', bins=12, alpha=1.0, zorder=3,
             ax=ax7)
sns.lineplot(x='cindex_lag0y', y='estimate__', data=df3_l,
             color='dimgray', ax=ax8)
ax8.fill_between(df3_l['cindex_lag0y'],
                 df3_l['lower__'], df3_l['upper__'],
                 color='dimgray',
                 alpha=0.30, edgecolor=None)
sns.lineplot(x='cindex_lag0y', y='estimate__', data=df3_h,
             color='red', ax=ax8)
ax8.fill_between(df3_h['cindex_lag0y'],
                 df3_h['lower__'], df3_h['upper__'],
                 color='red',
                 alpha=0.20, edgecolor=None)
ax8.yaxis.set_major_formatter(mticker.FormatStrFormatter('%.1f'))

ax7.yaxis.tick_right()
ax7.yaxis.set_label_position("right")
ax7.set_yticks([0, 0.1, 0.2])
ax7.set_yticklabels([0, 0.1, 0.2], fontsize=9)
ax7.set_ylabel("Observed\nproportion", ha='center', fontsize=9)
ax7.set_xlabel(r"SON-averaged DMI ($^{\circ}$C)", fontsize=11)
ax7.tick_params(axis='y', direction='in')
ax7.yaxis.set_label_coords(1.07, 0.05)

ax8.yaxis.tick_left()
ax8.yaxis.set_label_position("left")
ax8.tick_params(axis='y', direction='in')
ax8.set_ylabel('ACR (p.p.)', fontsize=11)
from matplotlib.ticker import MultipleLocator
ax8.yaxis.set_major_locator(MultipleLocator(4))

ax7.axvspan(+0.4, +2.5, color=colors[4], linewidth=0, alpha=0.10, zorder=0)
ax7.axvspan(-0.4, +0.4, color=colors[2], linewidth=0, alpha=0.00, zorder=0)
ax7.axvspan(-2.5, -0.4, color=colors[0], linewidth=0, alpha=0.10, zorder=0)

ax7.text(-0.04, 1.02, 'd', transform=ax7.transAxes, ha="center", va="center",
         fontsize=14, bbox=dict(boxstyle='square,pad=0.2',  # try 'square', 'round', 'larrow', etc.
            facecolor='white',          # box fill color
            edgecolor='black',          # box edge color
            linewidth=0.5))             # edge line width
...
ax8.legend(handles=handles, labels=labels,
           handler_map={tuple: HandlerTuple(ndivide=1)},
...
plt.savefig('/Users/tylerbagwell/Desktop/Main_fig3_v2.png', dpi=300, bbox_inches='tight', pad_inches=0.1)
plt.savefig('/Users/tylerbagwell/Desktop/Main_fig3_v2.pdf', dpi=300, format='pdf', bbox_inches='tight', pad_inches=0.1)
plt.show()
