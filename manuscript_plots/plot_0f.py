...
plt.scatter(group0[varname], group0[dvarname], 
            alpha=0.25, color='gray', edgecolor='gray', s=30, zorder=1, label=f'ENSO neutral onset-year (n={n0})')
plt.scatter(group1[varname], group1[dvarname], 
            alpha=0.5, color='red', edgecolor='red', s=30, marker='^', zorder=3, label=f'El Niño onset-year (n={n1})')
plt.scatter(group2[varname], group2[dvarname], 
            alpha=0.5, color='blue', edgecolor='blue', s=30, marker='v', zorder=3, label=f'La Niña onset-year (n={n2})')
...
plt.title('Onset-years among weakly-affected\nglobal grid cells, 1951-2023')

# plt.ylim(-3,3)
# plt.xlim(-3,3)

plt.tight_layout()
# plt.savefig('/Users/tylerbagwell/Desktop/justin_slidedeck/2/ENSO_weaklyaffected_prec_vs_diff_scatter.png', dpi=300, bbox_inches='tight', pad_inches=0.1)
plt.show()
...
# )
# print(group1[mask])
# group1[mask].to_csv("/Users/tylerbagwell/Desktop/filtered_group1.csv", index=False)
