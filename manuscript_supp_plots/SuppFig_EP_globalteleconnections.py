...
# teleA
pathA = '/Users/tylerbagwell/Documents/Rice_University/CCCV/data/panel_datasets/onset_datasets_state/Onset_Binary_GlobalState_EEItype2_wGeometry.csv'
...
# teleB
pathB = '/Users/tylerbagwell/Documents/Rice_University/CCCV/data/panel_datasets/onset_datasets_grid/Onset_Binary_Global_EEItype2_square4_wGeometry.csv'
...
# teleC
pathC = '/Users/tylerbagwell/Documents/Rice_University/CCCV/data/panel_datasets/onset_datasets_state/Onset_Binary_GlobalState_mrsosEEI_wGeometry.csv'
...
# teleD
pathD = '/Users/tylerbagwell/Documents/Rice_University/CCCV/data/panel_datasets/onset_datasets_grid/Onset_Binary_Global_mrsosEEI_square4_wGeometry.csv'
...
def draw_map(ax, var, label, cindex, tele_gdf, spatial_agg_type, cmap, nbin, threshold):
    ...
    if var!='teleconnection':
        if lab=='c':
            high = gdf_aggC[gdf_aggC['psi'] < threshold]
        elif lab=='d':
            high = gdf_aggD[gdf_aggD['psi'] < threshold]
        ax.add_geometries(high.geometry, crs=ccrs.PlateCarree(), facecolor='none', edgecolor='crimson', linewidth=0.6)

    #
    if cindex == 'NINO3':
        rect1 = mpatches.Rectangle((140,  -20),  40, 40,  # 140E→180
                           transform=ccrs.PlateCarree(),
                           fill=True, facecolor='blue', edgecolor='blue', linewidth=1.5, alpha=0.10)
        rect2 = mpatches.Rectangle((-180, -20), 100, 40,  # 180W→80W
                                transform=ccrs.PlateCarree(),
                                fill=True, facecolor='blue', edgecolor='blue', linewidth=1.5, alpha=0.10)
    ...
    if var=='teleconnection':
        ...
        hist_ax.text(0.55, 0.42, 'Drier in\nEP-El Niño\n↓', fontsize=8, ha="center", va="center", color='white',
                    bbox=dict(boxstyle='square,pad=0.2', linewidth=0, facecolor='gray', alpha=0.0),
                    transform=hist_ax.transAxes, rotation=0)
        hist_ax.text(0.55, 0.85, '↑\nWetter in\nEP-El Niño', fontsize=8, ha="center", va="center",
                    bbox=dict(boxstyle='square,pad=0.2', linewidth=0, facecolor='gray', alpha=0.0),
                    transform=hist_ax.transAxes, rotation=0)
    ...
    if var=='teleconnection':
        title_str = 'EP-ENSO Teleconnection, ' + spatial_agg_type
    else:
        title_str = 'Soil Moisture and ENSO E-Index, ' + spatial_agg_type
...
for ax, var, lab, cindex, tele_gdf, spatial_agg_type, cmap, nbin, threshold in zip(axes,
                                     ...
                                     ['PuRd', 'PuRd', 'cmo.curl_r', 'cmo.curl_r'], #PuOr
                                     [8,20,8,20],
                                     [0.49, 0.41, -0.05, -0.05]):
    draw_map(ax, var, lab, cindex, tele_gdf, spatial_agg_type, cmap, nbin, threshold)

plt.tight_layout()
plt.savefig('/Users/tylerbagwell/Desktop/SuppFig_EP_globalteleconnections.png', dpi=300, pad_inches=0.01)
plt.savefig('/Users/tylerbagwell/Desktop/SuppFig_EP_globalteleconnections.pdf', dpi=300, format='pdf', bbox_inches='tight', pad_inches=0.1)
plt.show()