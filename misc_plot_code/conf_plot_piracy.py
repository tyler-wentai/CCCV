...
path1 = '/Users/tylerbagwell/Desktop/panel_datasets/piracy_results/CE_ANOM_piracy_counts_NINO3_66.csv'
path2 = '/Users/tylerbagwell/Desktop/panel_datasets/piracy_results/CE_ANOM_piracy_counts_NINO3_90.csv'
path3 = '/Users/tylerbagwell/Desktop/panel_datasets/piracy_results/CE_ANOM_piracy_counts_NINO3_99.csv'

df1 = pd.read_csv(path1)
df2 = pd.read_csv(path2)
df3 = pd.read_csv(path3)

index_closest1 = df1['ANOM'].abs().idxmin()
index_closest2 = df2['ANOM'].abs().idxmin()
index_closest3 = df3['ANOM'].abs().idxmin()
val1 = 0#df1['estimate__'].iloc[index_closest1]
val2 = 0#df2['estimate__'].iloc[index_closest2]
val3 = 0#df3['estimate__'].iloc[index_closest3]
...
plt.plot(df1['ANOM'], df1['estimate__'], color='maroon', label='posterior mean', linewidth=2.0)
plt.fill_between(df1['ANOM'], df1['lower__'], df1['upper__'], color='r', alpha=0.30, edgecolor=None, label='likely (66% CI)')
plt.fill_between(df2['ANOM'], df2['lower__'], df2['upper__'], color='r', alpha=0.22, edgecolor=None, label='very likely (90% CI)')
plt.fill_between(df3['ANOM'], df3['lower__'], df3['upper__'], color='r', alpha=0.15, edgecolor=None, label='virtually certain (99% CI)')


# plt.axhline(y=0, color='gray', linestyle='--', linewidth=1, zorder=0)
# plt.axvline(x=0, color='gray', linestyle='--', linewidth=1, zorder=0)

plt.title('Global Piracy, 1994-2021 (N=108)')
plt.xlabel('NINO3 Index (s.d.)', fontsize=12)
plt.ylabel(r'P.P. annual piracy event count', fontsize=10)
plt.legend(loc=0, frameon=False)

plt.axvspan(+0.5, +3.1, color=colors[2], alpha=0.20, edgecolor='none', linewidth=0.0, zorder=0)
plt.axvspan(-0.5, +0.5, color=colors[1], alpha=0.00, edgecolor='none', linewidth=0.0, zorder=0)
plt.axvspan(-2.5, -0.5, color=colors[0], alpha=0.20, edgecolor='none', linewidth=0.0, zorder=0)

plt.text(0.0, 11, 'Neutral', fontsize=10, color='k', horizontalalignment='center')
plt.text(-1.1, 11, 'La Niña', fontsize=10, color='k', horizontalalignment='center')
plt.text(+1.825, 11, 'El Niño', fontsize=10, color='k', horizontalalignment='center')

plt.xlim(-1.7,3.12)

plt.tight_layout()
plt.savefig('/Users/tylerbagwell/Desktop/panel_datasets/piracy_results/Piracy_NINO3_Global.png', dpi=300, bbox_inches='tight', pad_inches=0.1)
plt.show()
