...
def compute_bymonth_partialcorr_map(ds1_in, ds2_in, climate_index, annualized_index, enso_index):
    ...
    for i in range(1,n_months+1):
        ...
        mean1_data = detrended1.mean(dim='valid_time', skipna=True)
        std1_data  = detrended1.std(dim='valid_time', skipna=True)
        var1_standardized = (detrended1 - mean1_data) / std1_data
        var1_standardized = var1_standardized.where(std1_data != 0, 0) # handle zero std cases

        mean2_data = detrended2.mean(dim='valid_time', skipna=True)
        std2_data  = detrended2.std(dim='valid_time', skipna=True)
        var2_standardized = (detrended2 - mean2_data) / std2_data
        var2_standardized = var2_standardized.where(std2_data != 0, 0) # handle zero std cases
